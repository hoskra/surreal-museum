...
void Scene::InitGameObjects()
{
    ...
    shaderProgram.program = l.CreateShader("data/shaders/color.vert", "data/shaders/color.frag");
    ...
    // reflector
    shaderProgram.reflectorPositionLocation = glGetUniformLocation(shaderProgram.program, "reflectorPosition");             CHECK_GL_ERROR();
    ...
    if (! l.loadSingleMesh("data/models/shoppingCart_0.obj"  , shaderProgram, &cart_0      )) std::cout << "Loading cart failed"      << std::endl;
    if (! l.loadSingleMesh("data/models/shoppingCart_1.obj"  , shaderProgram, &cart_1      )) std::cout << "Loading cart failed"      << std::endl;
    if (! l.loadSingleMesh("data/models/shoppingCart_2.obj"  , shaderProgram, &cart_2      )) std::cout << "Loading cart failed"      << std::endl;
    if (! l.loadSingleMesh("data/models/grass.obj"           , shaderProgram, &grass       )) std::cout << "Loading grass failed"     << std::endl;
    if (! l.loadSingleMesh("data/models/bush.obj"            , shaderProgram, &bush_0      )) std::cout << "Loading bush failed"      << std::endl;
    if (! l.loadSingleMesh("data/models/varhany.obj"         , shaderProgram, &varhany     )) std::cout << "Loading table failed"     << std::endl;
    if (! l.loadSingleMesh("data/models/stone_0.obj"         , shaderProgram, &stone_0     )) std::cout << "Loading stone0 failed"    << std::endl;
    if (! l.loadSingleMesh("data/models/stone_1.obj"         , shaderProgram, &stone_1     )) std::cout << "Loading stone1 failed"    << std::endl;
    if (! l.loadSingleMesh("data/models/stone_2.obj"         , shaderProgram, &stone_2     )) std::cout << "Loading stone2 failed"    << std::endl;

    if (! l.loadSingleMesh("data/models/painting.obj"        , shaderProgram, &painting_s  )) std::cout << "Loading paintingS failed" << std::endl;
    if (! l.loadSingleMesh("data/models/painting.obj"        , shaderProgram, &painting_h  )) std::cout << "Loading paintingH failed" << std::endl;
    if (! l.loadSingleMesh("data/models/painting.obj"        , shaderProgram, &painting_v  )) std::cout << "Loading paintingV failed" << std::endl;

    if (! l.loadSingleMesh("data/models/umbrella.obj"        , shaderProgram, &umbrella    )) std::cout << "Loading umbrella failed"        << std::endl;
    if (! l.loadSingleMesh("data/models/autopsy_0.obj"       , shaderProgram, &autopsy_0   )) std::cout << "Loading autopsy table0 failed"  << std::endl;
    if (! l.loadSingleMesh("data/models/autopsy_1.obj"       , shaderProgram, &autopsy_1   )) std::cout << "Loading autopsy table1 failed"  << std::endl;
    if (! l.loadSingleMesh("data/models/sewing.obj"          , shaderProgram, &sewing      )) std::cout << "Loading sewing machine failed"  << std::endl;
    
    if (! l.loadSingleMesh("data/models/petunia.obj"         , shaderProgram, &petunia      )) std::cout << "Loading petunia failed"  << std::endl;
    if (! l.loadSingleMesh("data/models/whale.obj"           , shaderProgram, &whale      )) std::cout << "Loading whale failed"  << std::endl;

    if (! l.loadSingleMesh("data/models/amanita_stem.obj"    , shaderProgram, &amanita_stem)) std::cout << "Loading amanita stem failed"  << std::endl;
    if (! l.loadSingleMesh("data/models/amanita_hat.obj"     , shaderProgram, &amanita_hat)) std::cout << "Loading amanita hat failed"  << std::endl;
    ...
    stone_0 -> m_position = glm::vec3(-15.45f, 8.85002f, -0.15f);
    stone_0 -> m_size = 10.0f;
               
    stone_1 -> m_position = glm::vec3(0.3f, 9.00002f, -0.25f);
    stone_1 -> m_size  = 10.0f;
    stone_1 -> m_angle = 180.0f;
    stone_1 -> m_direction = glm::vec3(0.0f, -1.0f, 0.0f);
               
    stone_2 -> m_position = glm::vec3(16.25f, 6.05001f, 0.45f);
...
void Scene::DrawCart(const glm::mat4& viewMatrix, const glm::mat4& projectionMatrix) {
    ...
    for (auto object : cart) {
        ...
        glUniform3fv(shaderProgram.reflectorPositionLocation, 1, glm::value_ptr(camera_position));
        glUniform3fv(shaderProgram.reflectorDirectionLocation, 1, glm::value_ptr(camera_direction));
...
void Scene::DrawAmanita(const glm::mat4& viewMatrix, const glm::mat4& projectionMatrix, float x, float y, float z) {
    ...
    modelMatrix = glm::translate(glm::mat4(1.0f), glm::vec3(
        amanita_stem->m_position.x,
        amanita_stem->m_position.y,
        amanita_stem->m_position.z
    ));
...
void Scene::SetMaterialUniforms(GameObject* object) {
    glUniform3fv(shaderProgram.diffuseLocation, 1, glm::value_ptr(object->m_diffuse));  // 2nd parameter must be 1 - it declares number of vectors in the vector array
    glUniform3fv(shaderProgram.ambientLocation, 1, glm::value_ptr(object->m_ambient));
    glUniform3fv(shaderProgram.specularLocation, 1, glm::value_ptr(object->m_specular));
